...
void static lcdwrite(enum typeOfWrite type, int message){
  if(type == COMMAND){
...
int bytes[7] = {0xFE ,0xEE, 0xC6, 0x82, 0xC6, 0xEE, 0xFE};
void fun(void){
	int i, j;
	for (j = 1; j <= 7; j++){
		lcdwrite(COMMAND,0x40);
		
		for (i = 0; i < 6; i++){
		lcdwrite(COMMAND,0x40+i);
			lcdwrite(COMMAND,0x80 + (j*8) );
			lcdwrite(DATA, 0xFF);
		}	
	}
}
